		...
		public function admin_options() {
			...
			<div id="wc_ifthen">
				...
				<div id="wc_ifthen_settings">
					<h2>
						<img src="<?php echo plugins_url( 'images/banner_payshop.png', __FILE__ ); ?>"/>
						<br/>
...
Email enviado automaticamente do plugin WordPress “Multibanco, MBWAY and Payshop (IfthenPay) for WooCommerce” para '.$to.' com CC para '.$cc;
		...
		public function get_icon() {
			...
			$icon_html = ( $this->get_option( 'small_icon' ) == 'yes' ? '<img src="'.esc_attr( $this->get_icon_url() ).'" alt="'.esc_attr( $alt ).'" width="18" height="24"/>' : '<img src="'.esc_attr( $this->get_icon_url() ).'" alt="'.esc_attr( $alt ).'" width="91" height="24"/>' );
		...
		function thankyou_instructions_table_html( $order_id, $order_total ) { //Missing Payshop email or phone number?
			$alt = ( WC_IfthenPay_Webdados()->wpml_active ? icl_t( $this->id, $this->id.'_title', $this->title ) : $this->title );
			...
			<table class="<?php echo $this->id; ?>_table" cellpadding="0" cellspacing="0">
				<tr>
					<th colspan="2">
						...
						<br/>
						<img src="<?php echo plugins_url( 'images/banner_payshop.png', __FILE__ ); ?>" alt="<?php echo esc_attr( $alt); ?>" title="<?php echo esc_attr( $alt); ?>"/>
					</th>
				</tr>
				<tr>
			...
			<table cellpadding="10" cellspacing="0" align="center" border="0" style="margin: auto; margin-top: 2em; margin-bottom: 2em; border-collapse: collapse; border: 1px solid #E60000; border-radius: 4px !important; background-color: #FFFFFF;">
				<tr>
					<td style="border: 1px solid #E60000; border-top-right-radius: 4px !important; border-top-left-radius: 4px !important; text-align: center; color: #000000; font-weight: bold;" colspan="2">
						<?php _e( 'Payment instructions', 'multibanco-ifthen-software-gateway-for-woocommerce' ); ?>
						<br/>
						<img src="<?php echo plugins_url( 'images/banner_payshop.png', __FILE__ ); ?>" alt="<?php echo esc_attr( $alt); ?>" title="<?php echo esc_attr( $alt); ?>" style="margin-top: 10px;"/>
					</td>
				</tr>
				<tr>
		...
		function email_instructions_payment_received( $order_id ) {
			...
			<p style="text-align: center; margin: auto; margin-top: 2em; margin-bottom: 2em;">
				<img src="<?php echo plugins_url( 'images/banner_payshop.png', __FILE__ ); ?>" alt="<?php echo esc_attr( $alt ); ?>" title="<?php echo esc_attr( $alt ); ?>" style="margin: auto;"/>
				<br/>
		...
		function disable_only_above_or_bellow( $available_gateways ) {
			return WC_IfthenPay_Webdados()->disable_only_above_or_bellow( $available_gateways, $this->id, WC_IfthenPay_Webdados()->payshop_min_value, WC_IfthenPay_Webdados()->payshop_max_value );
		}
